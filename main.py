...
# Составление массива координат полигонов
...
# Приведение объектов в тип данных полигонов
...
# Словарь дистанций до анализируемой области
...
# Разница площадей по абсолютному значению
...
# Pandas Time!!!!
# Создание DataFrame-а объектов с необходимыми признаками
...
# Результат вычислений по первой метрике
print(f'{'Метрика №1': ^72}')
print(f'Предполагаемое число жителей - {metric_one(df)}')
print(f'{'*' * 72}')
# Результат вычислений по второй метрике
print(f'{'Метрика №2': ^72}')
print(f'Предполагаемое число жителей - {metric_two(df)}')
plt.title('пофтафтье максямумь, позазуста! 😭', fontsize=24)
print(f'{'*' * 72}')
# Вычисление координат центроид объектов
cent = [(tuple(centroids[i].coords)[0]) for i in range(len(centroids))]
x_t = cent[0][0]
y_t = cent[0][1]
# print(tuple(centroids[0])[0])
# Нанесение текста на объекты по их центроидам со смещением вправо по оси абцисс
